...
import com.mysoftwarehouse.toolkit.demo.m80.MemberSelectionListing;
 ...
 * @author Ng Siak Hooi
 */
public class MemberMaintenance extends ModeEntryForm {

    @Override
    public void onInEnter() {
        String member = cmd.in.stringValue;
        if (!cmd.data.isNull(member)) {
            try {
                cmd.entry.setValue("member", member);
            } catch (EntryFieldWrongDataTypeException ex) {
                cmd.log.severe("wrong data type", ex);
            }
        }
    }
    ...
    public void finishModeAction() {
        cmd.mode.setAction(ModeSystemButton.Delete, null);
    ...
    public String getFormTitle() {
        return "MemberMaintenance.title";
    }
}
